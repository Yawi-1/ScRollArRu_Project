...
// ----------------------------------- Closed Tickets Component -----------------------------------
export default function ClosedTic() {
    // -------------------------- Database Connection -----------------------------------
    ...
    const markAsNotDone = (ticket) => {
        const docRef = doc(ticketRef, ticket.id);
        updateDoc(docRef, { ...ticket, status: true }).then(() => {
    ...
    return (
        <DashLayout>
            <div className="m-5">
                <div className="text-3xl mx-2 text-white my-2">Here are the Closed Tickets</div>
...

                {
                    tickets.length == 0 ? <div className="flex flex-col justify-center items-center ">
                        <h1 className="text-4xl font-bold text-center my-10 text-white">No Closed Tickets Here .....</h1>
                        <Link to='/dash/openTic' className="hover:bg-yellow-400 bg-yellow-500 p-2 rounded-md text-white font-bold">Check the pending tickets</Link>
                    </div> : <>
                        {tickets.map((ticket) => (
                            <div key={ticket.id} className=" min-h-40 border rounded-md bg-black/50 text-white flex flex-row w-full p-2 my-2 hover:bg-black/70">

                                {/* ----------------------------------- Col 1 ----------------------------------- */}
                                <div className="flex flex-col  w-2/3 justify-center mx-2 py-2">
                                    <img src={ticket.ticketPic} className=' w-3/5 rounded-md mx-4 text-2xl text-white' alt="No image attached!!" onClick={() => openModal(ticket.ticketPic)} />
                                </div>
                                {/* ----------------------------------- Col 2 ----------------------------------- */}
                                <div className="flex flex-col gap-2 w-2/3 justify-start mx-2">
                                ...
                                {/* ----------------------------------- Col 3 ----------------------------------- */}
                                <div className="flex flex-col gap-2 space-y-2 w-1/3 items-center justify-center ml-5">
                                    <div>
                                    ...
                                    <div>
                                        <label htmlFor={ticket.id} className="relative inline-flex items-center cursor-pointer">
                                            ...
                                            <span className="ml-3 text-sm font-medium text-red-600">Mark as Pending</span>
                                        </label>
                                ...
                                {modalVisible && (
                                    <div className="fixed inset-y-0   my-24 flex justify-center w-2/3">
                                        <div className="p-1 rounded-lg ">
                                            <img src={modalImage} alt="Magnified view" className="max-w-11/12 max-h-1/2 no-repeat" />
